<?php
class Catalog_UserVideoController extends Zend_Controller_Action
{
	...
	public function deleteAction()
	{
      ...
      'ids' => array('NotEmpty', 'Int')
		...
		if ($input->isValid()) {
			...
			$this->_redirect('/user/catalog/video/success');
		} else {
			throw new Zend_Controller_Action_Exception('Invalid input');
		}
	}
	...
	{
		// generate input form
		$form = new Webteam_Form_VideoUpdateUser;
		$this->view->form = $form;
		...
		if ($this->getRequest()->isPost()) {
			...
			if ($form->isValid($postData)) {
				...
				$q = Doctrine_Query::create()
				->update('Webteam_Model_Video u')
				->set('u.Title', '?', $input['Title'])
				->set('u.Description', '?', $input['Description'])
				->addWhere('u.VideoID = ?',  $input['VideoID']);
				$q->execute();
	...
	public function successAction()
	{
		if ($this->_helper->getHelper('FlashMessenger')->getMessages()) {
			...
			$this->_redirect('/user/catalog/video/index');
		}
	}
